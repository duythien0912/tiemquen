...
def test_seller_pwa_shell_served(client):
    r = client.get("/seller/")
    assert r.status_code == 200
    assert "manifest.json" in r.text and "/webapp/" in r.text  # React bundle shell
    # PWA assets served from the seller/ static mount
    for path in ("/seller/manifest.json", "/seller/sw.js", "/seller/icon.svg"):
